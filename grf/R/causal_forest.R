...
#'
#' @export
causal_forest <- function(X, Y, W, Q, Q2, Q3, Q4,
                          Y.hat = NULL,
                          W.hat = NULL,
                          sample.fraction = 0.5,
                          mtry = NULL,
    ...
    if(length(Q) != nrow(X)) { stop("Q has incorrect length.") }
    if(length(Q2) != nrow(X)) { stop("Q2 has incorrect length.") }
    if(length(Q3) != nrow(X)) { stop("Q3 has incorrect length.") }
    if(length(Q4) != nrow(X)) { stop("Q4 has incorrect length.") }
    ...
    if (is.null(Y.hat)) {
      forest.Y <- regression_forest(X, Y, Q, sample.fraction = sample.fraction, mtry = mtry, tune.parameters = tune.parameters,
    ...
    if (is.null(W.hat)) {
      forest.W <- regression_forest(X, W, Q, sample.fraction = sample.fraction, mtry = mtry, tune.parameters = tune.parameters,
                                    num.trees = min(500, num.trees), num.threads = num.threads, min.node.size = NULL, 
                                    honesty = TRUE, seed = seed, ci.group.size = 1, alpha = alpha, imbalance.penalty = imbalance.penalty,
                                    clusters = clusters, samples_per_cluster = samples_per_cluster);
      W.hat <- predict(forest.W)$predictions
    ...
    data <- create_data_matrices(X, Y.centered, W.centered, Q, W, Y, Q2, Q3, Q4)
