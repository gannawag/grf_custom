...
std::vector<Prediction> OptimizedPredictionCollector::collect_predictions(const Forest& forest,
  ...
  for (size_t sample = 0; sample < num_samples; ++sample) {
    ...
    if (num_leaves == 0) {
      ...
      predictions.push_back(Prediction(nan, nan, nan, nan, nan, nan, nan, nan));
      continue;
    }
    ...
    std::vector<double> q = strategy->q_effect(prediction_values);
    std::vector<double> yc = strategy->yc_effect(prediction_values);
    std::vector<double> q2 = strategy->q2_effect(prediction_values);
    std::vector<double> q3 = strategy->q3_effect(prediction_values);
    std::vector<double> q4 = strategy->q4_effect(prediction_values);
    ...
    Prediction prediction(point_prediction, variance, mse, q, yc, q2, q3, q4);
